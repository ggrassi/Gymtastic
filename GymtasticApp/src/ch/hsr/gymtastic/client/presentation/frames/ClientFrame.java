...
public class ClientFrame implements Observer {
	...
	EventQueue.invokeLater(new Runnable() {
	    public void run() {
		try {
		    ClientFrame window = new ClientFrame(squadController, networkController);
     ...
     *            the network controller
     */
    public ClientFrame(SquadController squadController, NetworkClientController networkController) {
	...
	try {
	    ...
	    this.networkController.setGymCupInfoController(gymCupInfoController);
	    this.competitionInfoController = new CompetitionInfoController();
	    this.networkController.setRoundInfoController(competitionInfoController);
	...
	panelOverview = new OverviewPanel(gymCupInfoController, competitionInfoController, squadController, deviceType,
		this);
	...
	if (tabbedPane.getComponents().length == 1) {
	    ...
	    panelActualSquad = new ActualSquadPanel(actualSquad, deviceType, networkController, this);
	...
	if (gymCupInfoController.getGymCupClientInfo() != null) {
	    deviceType = gymCupInfoController.getGymCupClientInfo().getDeviceType();
	}
    ...
    /**
     * Ends a round on the client and to wait on the new Squad send from the
     * server.
     */
