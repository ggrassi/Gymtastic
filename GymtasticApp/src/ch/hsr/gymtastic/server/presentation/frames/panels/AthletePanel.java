...
public class AthletePanel extends JPanel implements Observer {
	...
	tableAthletes.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
	...
	panelStatisticsBorder.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Statistik",
	...
	panelAthletesBorder.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Athleten",
		TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
	...
	if (!txtFieldSearchAthlete.getText().equals("Suchen...")) {
	    rowFilter = RowFilter.regexFilter("(?i)" + Pattern.quote(txtFieldSearchAthlete.getText()));
	}
