...
public class SquadsSelectionFrame {
	...
	squadSelectionFrame.getContentPane().add(scrollPaneSquads, BorderLayout.CENTER);

	tableSquads = new JTable();
	squadSelectionTableModel = new SquadSelectionTableModel(gymCupController);
	tableSquads.setModel(squadSelectionTableModel);
	scrollPaneSquads.setViewportView(tableSquads);

	JPanel panelSouth = new JPanel();
	squadSelectionFrame.getContentPane().add(panelSouth, BorderLayout.SOUTH);
	...
	tableSquads.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
	...
	for (int i : rows) {
	    ...
	    Squad selectedSquad = gymCupController.getGymCup().getUnallocatedSquads().get(modelRow);
