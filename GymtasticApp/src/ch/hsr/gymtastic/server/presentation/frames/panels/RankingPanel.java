...
*/
	...
	btnSave.addActionListener(new ActionListener() {
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			PdfRankingTableExporter pdfRankingTableExporter = new PdfRankingTableExporter(gymCupController
				.getGymCup(), chooser.getSelectedFile().getAbsolutePath());

			if (comboBoxRankingModel.getSelectedItem().toString().equalsIgnoreCase("Alle")) {

			    try {
				pdfRankingTableExporter.createTotalRankingList();
			    ...
			    try {
				pdfRankingTableExporter.createProgramClassRankingList(comboBoxRankingModel
			...
			PdfStartlistExporter pdfStartlistExporter = new PdfStartlistExporter(gymCupController
				.getGymCup(), chooser.getSelectedFile().getAbsolutePath());
			if (comboBoxStartlistModel.getSelectedItem().toString().equalsIgnoreCase("Alle")) {
			    ...
			    try {
				pdfStartlistExporter.createCompetitionStartlist(comboBoxStartlistModel
					.getSelectedItem().toString());
			    } catch (FileNotFoundException e1) {
				e1.printStackTrace();
			    } catch (DocumentException e1) {
	...
	panelStartlist.setBorder(new TitledBorder(null, "Startliste", TitledBorder.LEADING, TitledBorder.TOP, null,
		null));
	...
	gbl_panelStartlist.columnWeights = new double[] { 0.0, 1.0, Double.MIN_VALUE };
	...
	panelRankingTable.setBorder(new TitledBorder(null, "Rangliste", TitledBorder.LEADING, TitledBorder.TOP, null,
		null));
	...
	gbl_panelRankingTable.columnWeights = new double[] { 0.0, 1.0, Double.MIN_VALUE };
	...
	panelControl.setBorder(new TitledBorder(null, "Document Verwaltung", TitledBorder.LEADING, TitledBorder.TOP,
		null, null));
	...
	gbl_panelControl.columnWeights = new double[] { 0.0, 0.0, Double.MIN_VALUE };
	...
	panelPreview.setBorder(new TitledBorder(null, "Vorschau", TitledBorder.LEADING, TitledBorder.TOP, null, null));
	...
	comboBoxStartlist.setModel(comboBoxStartlistModel);
	comboBoxRankingModel = new ProgramClassComboBoxModel(gymCupController.getGymCup().getProgramClasses());
	comboBoxRankingTable.setModel(comboBoxRankingModel);
    }
