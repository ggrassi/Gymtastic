...
public class PdfRankingTableExporter extends PdfExporter {
     ...
     */
    public void createTotalRankingList() throws FileNotFoundException, DocumentException {
	createFile();
	writeTotalContent();
	closeFile();
    }
     ...
     *             the document exception
     */
    public void createProgramClassRankingList(String programClass) throws FileNotFoundException, DocumentException {
	...
	for (Athlete athlete : rankingList) {
		...
		table.addCell(finalMarkFormat.format(athlete.getMarks().get(deviceType).getFinalMark()));
	    }
	...
	Paragraph title = new Paragraph("Rangliste vom " + gymCup.getName() + " in " + gymCup.getLocation() + " vom "
		...
		+ DateFormatConverter.convertDateToString(gymCup.getEndDate()), totalRankingTitleFont);
     ...
     *             the document exception
     */
    private void writeProgramClassTitle(String programClass) throws DocumentException {
	Paragraph title = new Paragraph("Programm Klasse " + programClass, programClassRankingTitleFont);
	...
	public int compare(Athlete athlete1, Athlete athlete2) {
		...
		return 1;
	    } else if (athlete1.getSumOfEndMarks() > athlete2.getSumOfEndMarks()) {
		return 0;
	    } else {
		return 0;
	    }
